...
class CausalDiffusionInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        for module in [self.generator.model, self.text_encoder, self.vae]:
    ...
    def inference(
        ...
        if initial_latent is not None:
            ...
            # cache_start_frame == initial_latent.shape[1]

        # Step 3: Temporal denoising loop
        # num_frames == noise.shape[1]  #  noise (bs, output_len)
        # num_blocks == noise.shape[1] // self.num_frame_per_block
        # num_input_frames == initial_latent.shape[1]
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache_pos.append({
                "k": torch.zeros([batch_size, kv_cache_size, 40, 128], dtype=dtype, device=device), # 12 for 1.3B, 40 for 14B
    ...
    def _initialize_crossattn_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            crossattn_cache_pos.append({
                "k": torch.zeros([batch_size, 512, 40, 128], dtype=dtype, device=device),    # 12 for 1.3B, 40 for 14B
                "v": torch.zeros([batch_size, 512, 40, 128], dtype=dtype, device=device),
                "is_init": False
            })
            crossattn_cache_neg.append({
                "k": torch.zeros([batch_size, 512, 40, 128], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, 512, 40, 128], dtype=dtype, device=device),
                "is_init": False
            })
