...
parser.add_argument("--duration", type=int, default=1, help="the whole duration time of generated video")
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    # Generate 81 frames
    image = batch['image'].squeeze(0).unsqueeze(0).unsqueeze(2).to(device=device, dtype=torch.bfloat16)
    pipeline.vae = pipeline.vae.to(image.device)
    initial_latent = pipeline.vae.encode_to_latent(image).to(device=device, dtype=torch.bfloat16)
    initial_latent = initial_latent.repeat(args.num_samples, 1, 1, 1, 1)
    ...
    for rollout_index in range(num_rollout):
        ...
        print('vid_test_normalized:', vid_test_normalized.shape)
