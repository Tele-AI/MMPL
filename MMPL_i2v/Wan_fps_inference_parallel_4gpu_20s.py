...
#     pipeline.generator_uncond.load_state_dict(state_dict['generator' if not args.use_ema else 'generator_ema'])
if args.checkpoint_path:
    state_dict = torch.load(args.checkpoint_path, map_location="cpu")
    weight_key = 'generator_ema' if args.use_ema else 'generator'
    # print(5555, state_dict)
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    if args.i2v:
        ...
        pipeline_chunk_1.vae = pipeline_chunk_1.vae.to("cuda")
        initial_latent = pipeline_chunk_1.vae.encode_to_latent(image).to(device=device, dtype=torch.bfloat16)
    ...
    # Generate 81 frames
    image = batch['image'].squeeze(0).unsqueeze(0).unsqueeze(2).to(device=device, dtype=torch.bfloat16)
    pipeline_chunk_1.vae = pipeline_chunk_1.vae.to(image.device)
    initial_latent = pipeline_chunk_1.vae.encode_to_latent(image).to(device=device, dtype=torch.bfloat16)
    image_initial_latent = initial_latent.repeat(args.num_samples, 1, 1, 1, 1)
    # torch.save(initial_latent, 'image.pt')
    ...
    def generate_video_chunk(pipeline, sampled_noise, prompts, chunk_idx, args, wait_file=None):
        if wait_file:
            ...
            initial_latent = image_initial_latent
