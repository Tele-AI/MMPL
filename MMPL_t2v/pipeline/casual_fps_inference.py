...
class CausalFPSInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        all_num_frames = [2, 7, 6, 6]
        result = [[i for i, v in enumerate(clean_steps) if v == target] for target in range(4)]
        ...
        for current_num_frames in all_num_frames:
            ...
            if (initial_latent is None) or (global_chunk_index != 0):
                ...
                ''' add casual between anchor parts'''
                target_values = [31200, 29640]
                if global_chunk_index == 2:
                    
                    '''++++++++++++++++++++++++++++++++'''
                    latents[:, 0:1] = self.ddpm_scheduler.add_noise(
                        output[:, 3:4].flatten(0, 1).to(latents.device),
                        torch.randn_like(latents[:, 0:1]).flatten(0, 1).to(latents.device),
                        self.ddmp_timestep.flatten(0, 1).to(latents.device)
                    ).unflatten(0, (batch_size, 1))

                    latents[:, -1:] = self.ddpm_scheduler.add_noise(
                        output[:, 10:11].flatten(0, 1).to(latents.device),
                        torch.randn_like(latents[:, -1:]).flatten(0, 1).to(latents.device),
                        self.ddmp_timestep.flatten(0, 1).to(latents.device)
                    ).unflatten(0, (batch_size, 1))
                    '''++++++++++++++++++++++++++++++++'''

                    # Remove all target_values from all attention_vis_index lists if present
                    for cache in [self.kv_cache_pos, self.kv_cache_neg]:
                        for block in cache:
                            for val in target_values:
                                if val in block["attention_vis_index"]:
                                    block["attention_vis_index"].remove(val)

                elif global_chunk_index == 3:
                    
                    '''++++++++++++++++++++++++++++++++'''
                    latents[:, 0:1] = self.ddpm_scheduler.add_noise(
                        output[:, 12:13].flatten(0, 1).to(latents.device),
                        torch.randn_like(latents[:, 0:1]).flatten(0, 1).to(latents.device),
                        self.ddmp_timestep.flatten(0, 1).to(latents.device)
                    ).unflatten(0, (batch_size, 1))

                    latents[:, -1:] = self.ddpm_scheduler.add_noise(
                        output[:, 19:20].flatten(0, 1).to(latents.device),
                        torch.randn_like(latents[:, -1:]).flatten(0, 1).to(latents.device),
                        self.ddmp_timestep.flatten(0, 1).to(latents.device)
                    ).unflatten(0, (batch_size, 1))
                    '''++++++++++++++++++++++++++++++++'''

                    # Add all target_values to all attention_vis_index lists if not already present
                    for cache in [self.kv_cache_pos, self.kv_cache_neg]:
                        for block in cache:
                            for val in target_values:
                                if val not in block["attention_vis_index"]:
                                    block["attention_vis_index"].append(val)
                ''' add casual between anchor parts'''
                ...
                if global_chunk_index == 1:
                    ...
                    print("🐱🐱🐱 save_latents ", save_latents.sum(), " 🐱🐱🐱 self.save ", self.save)
                ...
                current_start_frame = result[0]
                cache_start_frame = result[0]
                ...
                output = output.to(self.device_cond)
                output[:, result[global_chunk_index]] = initial_latent.to(self.device_cond)
