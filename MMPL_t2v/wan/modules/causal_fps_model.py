...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if 15 * 1560 not in current_start_list:
                if 19 * 1560 not in current_start_list:
                    ...
                    calculate = [x - (6 * 1560) if x >= (19 * 1560) else x for x in kv_cache["attention_vis_index"]]
                    all_indices = [idx for start in calculate for idx in range(start, start + frame_seqlen)]
                    ...
                    for i, start in enumerate(current_start_list):
                        ...
                        if i == 5 or i == 6:
                            start = start - frame_seqlen * 6
                            end = end - frame_seqlen * 6
                        else:
                            pass
                ...
                kv_cache["attention_vis_index"] = list(set(kv_cache["attention_vis_index"]))
                calculate = [x - (6 * 1560) if x >= (19 * 1560) else x for x in kv_cache["attention_vis_index"]]
                ...
                x = attention(
                    roped_query,
                    torch.cat([kv_cache["k"][:, all_indices], roped_key], dim=1),
                    torch.cat([kv_cache["v"][:, all_indices], v], dim=1),
                )
